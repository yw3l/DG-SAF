...
def train(args):
    ...
    if not args.no_cuda and torch.cuda.is_available():
        ...
        print(f"Using device: {device} (Number of GPUs: {n_gpu})")
        # 如果只有一个GPU，DataParallel仍然可以工作，但不会带来收益
        # 如果没有GPU或禁用CUDA，则使用CPU
    else:
    ...
    valid_dataset = KGDataset(
        ...
        data['entity2id'], data['relation2id'],
        len(data['entity2id']), 0, 'valid'
    )
    test_dataset = KGDataset( # Add test dataset for final evaluation
        data['test'], data['entity_descriptions'],
        data['entity2id'], data['relation2id'],
        len(data['entity2id']), 0, 'test'
    )

    # 创建数据加载器 - 修改部分：考虑多GPU调整有效batch size
    # 注意：DataParallel 会将 batch 分散到各个 GPU，所以每个 GPU 的实际 batch size 是 args.batch_size
    # 如果你想保持总的 batch size 不变，可以将 args.batch_size 除以 n_gpu
    # 但通常我们会增加总的 batch size 以利用多GPU
    effective_batch_size = args.batch_size * max(1, n_gpu) # 粗略估计总的处理能力
    print(f"Effective batch size (approx): {effective_batch_size}")
    train_loader = DataLoader(
        train_dataset, batch_size=args.batch_size, # 这是每个GPU的batch size
        shuffle=True, num_workers=args.num_workers, pin_memory=True if n_gpu > 0 else False # Pin memory if using GPU
    )
    ...
    # 修改部分：使用 DataParallel 包装模型
    if n_gpu > 1:
        print(f"Using {n_gpu} GPUs with DataParallel")
        model = torch.nn.DataParallel(model) # Wrap the model

    model.to(device) # Move the (potentially wrapped) model to device(s)

    # 优化器 - 注意：优化器应该在模型包装之后，移动到设备之后创建
    # 获取需要优化的参数
    # 对于 DataParallel 包装的模型，.module 访问原始模型
    model_to_optimize = model.module if n_gpu > 1 else model

    # 确保 BERT 的可训练参数也被加入优化器
    bert_params = []
    other_params = []
    ...
    optimizer = optim.AdamW([
        {'params': other_params, 'lr': args.text_lr}, # Use a base LR, adjust as needed
        {'params': bert_params, 'lr': args.text_lr}, # Fine-tune BERT params
        # 如果你想为不同部分设置不同学习率，可以更细致地划分
        # {'params': model_to_optimize.text_encoder.projection.parameters(), 'lr': args.text_lr},
        # {'params': model_to_optimize.text_encoder.bert.encoder.layer[-2:].parameters(), 'lr': args.text_lr},
        # {'params': model_to_optimize.geometry_param.parameters(), 'lr': args.param_lr},
        # {'params': model_to_optimize.entity_real.parameters(), 'lr': args.struct_lr},
        # {'params': model_to_optimize.entity_imag.parameters(), 'lr': args.struct_lr},
        # {'params': model_to_optimize.text_to_struct.parameters(), 'lr': args.text_lr},
    ...
    for epoch in range(args.epochs):
        ...
        for batch in progress_bar:
            # h_ids, r_texts, t_ids, t_neg_ids = batch # 原始 unpacking
            # 修改部分：处理 batch unpacking，注意 None 的处理
            h_ids, r_texts, t_ids, t_neg_ids_raw, h_descs, t_descs = batch

            # 移动到设备
            h_ids = h_ids.to(device)
            t_ids = t_ids.to(device)
            # t_neg_ids 可能是 None 或 Tensor
            t_neg_ids = None
            if t_neg_ids_raw is not None and t_neg_ids_raw.numel() > 0:
                t_neg_ids = t_neg_ids_raw.to(device)
            # r_texts, h_descs, t_descs 是列表，留在CPU上由 TextEncoder 处理

            # 前向传播 - 修改部分：DataParallel 会自动处理输入的分发
            optimizer.zero_grad()
            # 确保传递所有需要的参数给模型的 forward 方法
            # 你的原始 DGSAF forward 只接受 h_ids, r_texts, t_ids, t_neg_ids
            # 如果你想用 h_descs, t_descs，需要修改 DGSAF 的 forward
            # 目前我们按原始 DGSAF forward 签名传递参数
            # loss, (struct_loss, text_loss, consist_loss) = model(
            #     h_ids, r_texts, t_ids, t_neg_ids
            # )
            # 如果 DGSAF forward 已修改为接受 h_descs, t_descs，则使用下面这行
            # 并确保模型 forward 方法正确处理它们
            # loss, (struct_loss, text_loss, consist_loss) = model(
            #     h_ids, r_texts, t_ids, t_neg_ids, h_descs, t_descs
            # )

            # 由于你提供的代码中 DGSAF forward 签名是旧的，我们先用旧的
            # 但你需要确认你的 DGSAF forward 是否已更新
            try:
                 loss, (struct_loss, text_loss, consist_loss) = model(
                     h_ids, r_texts, t_ids, t_neg_ids # Add h_descs, t_descs if DGSAF updated
                 )
            ...
            # 反向传播
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.max_grad_norm) # Clip on the wrapped model
            optimizer.step()
            scheduler.step()

            # 记录损失 (DataParallel 返回的 loss 是所有 GPU 损失的平均值)
        ...
        # 验证 - 修改部分：确保 evaluate 函数也能处理多GPU模型
        print("Starting validation...")
        # 传递原始模型给 evaluate，因为它内部不需要 DataParallel 的包装
        mrr, hits = evaluate(
            model.module if n_gpu > 1 else model, valid_dataset, device, # Pass the original model
    ...
    # Load best model
    model_to_load = model.module if n_gpu > 1 else model
    model_to_load.load_state_dict(torch.load(os.path.join(args.output_dir, 'best_model.pt')))
    # Evaluate on test set
    test_mrr, test_hits = evaluate(
        model_to_load, test_dataset, device, # Use test_dataset and loaded model
        len(data['entity2id']), args.hits_at_k
    )
    print("\nTest Results:")
